...
- Ignore native deletion events for temporary and excluded files before accessing sync state, and wait for durable completion in download-order tests to prevent Windows database cleanup races.
...
- Made D-Bus disposal checks deterministic by accepting documented queued-call shutdown outcomes and waiting for confirmed tray-name withdrawal.
...
- Preserved fair upload retries when using timestamp ordering: first attempts precede retries, and upload progress remains separate from downloads between batches.
...
- Replaced Current sync and the separate pending-file count with Sync status. Inactivity is shown without file counts; active files and sync errors show processed and total file counts. Progress continues across successive upload batches while the ready queue remains nonempty.
...
- **`tools/BrandAssetGen/BrandAssetGen.csproj` GenerateBrandAssets target.**
  ...
  / App.Linux.csproj propagate `--runtime <RID>` into the
