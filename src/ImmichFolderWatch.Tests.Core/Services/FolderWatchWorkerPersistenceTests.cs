...
public sealed class FolderWatchWorkerPersistenceTests
{
    ...
    public async Task UploadAll_UsesGlobalTransferOrderAcrossBatches(string order, string first, string second)
    {
        ...
        Assert.Equal(new[] { first, second }, client.UploadedFiles.ToArray());
    }
    ...
    private sealed class RecordingAssetClient : IImmichAssetClient
    {
        ...
        public System.Collections.Concurrent.ConcurrentQueue<string> UploadedFiles { get; } = new();

        public System.Collections.Concurrent.ConcurrentQueue<string> DownloadedAssets { get; } = new();
        ...
        {
            UploadedFiles.Enqueue(Path.GetFileName(request.FilePath));
