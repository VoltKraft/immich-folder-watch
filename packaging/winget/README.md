...
## One-Time Catch-Up To `1.4.0`

1. Open the GitHub Actions workflow `WinGet`.
2. Run the workflow manually with `release_tag` set to `v1.4.0`.
3. Wait for the automation to submit the WinGet update for:

```text
https://github.com/VoltKraft/immich-folder-watch/releases/download/v1.4.0/immich-folder-watch-1.4.0-win-x64.msi
```
...
After the `1.4.0` catch-up submission, later published GitHub releases trigger `.github/workflows/winget.yaml` automatically and submit the next WinGet update PR.
